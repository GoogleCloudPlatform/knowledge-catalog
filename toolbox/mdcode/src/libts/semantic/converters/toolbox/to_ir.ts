...
//   tool `parameters`            -> action parameters, `@name`-bound, carrying
...
//   - What a Toolbox parameter says beyond name, type, description, optionality
//     and default: its allowed and excluded values, and whether it is filled
//     from a verified claim rather than by the caller. The first four DO carry
//     (ActionParameter gained them in #442), and the description is the one
//     that mattered -- it is the sentence the agent reads to decide what to put
//     in the argument. A value restriction belongs in a constraint guarding the
//     action, which the warning says; a parameter the caller must not supply
...
function toParameter(
  ...
  // `required` is the one field whose absence means something: Toolbox's
  // `GetRequired` defaults a nil pointer to TRUE, the opposite of how an absent
  // flag usually reads, so it is written out rather than left off.
  //
  // A defaulted parameter states neither, because OSI rejects `required: true`
  // beside a `default` -- a value that is always supplied cannot also be
  // demanded -- and derives optionality from the default's presence.
  return {
    name: p.name,
    type: type ?? 'Opaque',
