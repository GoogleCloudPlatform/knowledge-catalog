...
cli.command('push', 'Push catalog entries')
   ...
   .option('--transpile', 'Rewrite vendor-dialect (e.g. Snowflake/Databricks) expressions to GoogleSQL via sqlglot before deploying, filling target expressions the loader left unset; semantic-model push only')
   .action(async (options) => {
      let exitCode = 1;
      try {
