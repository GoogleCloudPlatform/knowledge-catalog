...
cli.command('pull', 'Pull catalog entries').action(async () => {
  let exitCode = 1;
  try {
...
cli.command('push', 'Push catalog entries')
    ...
    .option(
        '--target <targets>',
        'Semantic-model push destination(s): bq, kc, all (default), or a comma-separated list (e.g. bq,kc)')
    .option(
        '--print',
        'Print each pushed destination\'s generated artifact in its native format (BigQuery Graph SQL DDL, Knowledge Catalog entry plan); scope with --target (semantic-model push only)')
    .option(
        '--transpile',
