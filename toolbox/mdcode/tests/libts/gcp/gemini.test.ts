...
describe('where the Gemini judge sends its request', () => {
  test(
      'uses a region that serves Gemini rather than the Compute Engine one',
      () => {
        ...
        expect((judge as any)._endpoint)
            .toBe(
                `https://${DEFAULT_JUDGE_LOCATION}-aiplatform.googleapis.com`);
  ...
  test(
      'still names `global` as the location in the resource path', async () => {
  ...
  test(
      'names the project, the region and the model in the resource path',
      async () => {
        const judge = new GeminiJudge(CTX);
        const {resource} = await ask(judge, answering(OK));
        expect(resource).toBe(
            `projects/test-project/locations/${DEFAULT_JUDGE_LOCATION}` +
            `/publishers/google/models/${DEFAULT_JUDGE_MODEL}:generateContent`);
      });
...
describe('what the Gemini judge asks', () => {
  ...
  test(
      'switches thinking off, because a caller is waiting on the guard',
      async () => {
  ...
  test(
      'asks the default model the same way however it was chosen', async () => {
        // Naming the model the flag would have defaulted to must not change the
        // request. Keying on whether a model was named rather than on which
        // model it is would leave thinking on for this spelling alone.
...
describe('what the Gemini judge makes of an answer', () => {
  ...
  test(
      'JSON that does not say whether the rule holds is not a verdict',
      async () => {
        const judge = new GeminiJudge(CTX);
...
describe('what a judge is offered', () => {
  test('is offered nothing and asks once', async () => {
    // A judge settles a rule from the call and nothing else: it declares no
    // tool, makes one request, and a rule it cannot settle that way is one it
    // reports it cannot settle.
    const judge = new GeminiJudge(CTX);
