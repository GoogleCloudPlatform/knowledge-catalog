...
describe('the Toolbox AlloyDB quickstart imports with its edges', () => {
  ...
  test('a Toolbox parameter type becomes an OSI datatype', () => {
    expect(action(model, 'add-to-cart').parameters).toEqual([
      {name: 'cart_id', type: 'Integer', isEntityRef: false},
      {name: 'product_id', type: 'Integer', isEntityRef: false},
      {name: 'quantity', type: 'Integer', isEntityRef: false},
      {name: 'price', type: 'Float', isEntityRef: false},
    ]);
  });
...
describe('what a configuration cannot say', () => {
  ...
  test('a parameter keeps its name and type and loses everything else', () => {
    // Every parameter in both fixtures carries a description -- it is what the
    // agent reads to decide what to put in the argument -- and an OSI action
    // parameter is a name and a type.
    expect(matching(hotels.warnings, 'parameter descriptions are dropped'))
        .toHaveLength(1);
  });
