...
- `PlaceOrder` -- Action 'PlaceOrder' is guarded by 'OrderWithinCustomerCredit', which is settled by reading the call, and this runtime was given no judge to ask. Running it would apply a write the model says must be checked first, so it is refused rather than run unchecked.
