...
`python -m eval --run` uses this to match the internal Evaluation tab: a golden
...
# The published agent lives next to the eval package: toolbox/enrichment/src.
...
async def run_agent(inputs: dict, project: str, model: str, output_dir: str,
  ...
  # Run from the eval-invocation dir (toolbox/enrichment) so relative inputs like
