"""Cross-run aggregation for `--runs > 1`, mirroring the internal harness.

IMPORTANT — keep in sync with the canonical roll-up in the internal
`enrichment_agent_eval/harness/compare.py` (`_metric_summary`,
`_consistency_metrics`, `_runs_detail`). The public eval reuses the SAME shapes so
the CLI matches the webapp Evaluation tab:
  ...
  - two informational cross-run STABILITY metrics — `concept_consistency`
    ...
    are **n/a (None) unless there are ≥2 runs**, exactly like the harness. They
    never gate the case and are excluded from the average.
"""
...
def metric_rollup(run_results: list[dict]) -> list[dict]:
  """Per-metric roll-up across a case's runs (mirrors compare._metric_summary):
...
def consistency_metrics(run_results: list[dict], judge) -> list[dict]:
  """Two cross-run STABILITY metrics (ported from compare._consistency_metrics),
  ...
  matching the harness, since consistency is undefined for a single run)."""
...
def runs_detail(run_results: list[dict]) -> list[dict]:
  """Per-run drill-down (mirrors compare._runs_detail): each run's metrics +
...
def aggregate_runs(run_results: list[dict], mode: str | None = None,
  ...
  one case result that matches the internal harness shape: per-metric mean +
