...
async def run(topic: str, docs: list[str], folder: str | None, output_dir: str | None,
    ...
    # entry_group is `project.location.entryGroupId`; derive the entry type +
    # resource-name prefix for the generated KB entries (all customer-supplied,
    # nothing hardcoded).
    ...
    entry_type = f"{eg_project}.{eg_location}.simple-entry-type"
    resource_name_prefix = f"projects/{eg_project}/locations/{eg_location}/catalog"

    # Scaffold the KB manifest up front with `kcmd init --kb` (clean output dir),
    # so the agent talks to the catalog only through kcmd. The LLM then generates
    # ONLY the entries. Falls back to a deterministic manifest if init can't reach
    # the catalog (e.g. a brand-new entry group).
    if output_dir:
        ok, msg = kcmd_tools.init_kb(output_dir, entry_group)
        print(f"[kcmd] init --kb {entry_group}: {'OK' if ok else 'FAILED'} {msg}", flush=True)
    ...
    # (catalog.yaml was already scaffolded via `kcmd init --kb` at the start.)
