...
|                            |                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------ |
| **Status**                 | Community extension, proposed                                                                          |
...
OKF cross-links between concepts are untyped: `[B](../b.md)` carries no relationship and no lifecycle meaning. v0.2 (#227) adds a concept-level lifecycle — `status: draft | stable | deprecated` (§5.4) — but no way to say *what replaced* a deprecated concept, or *what disputes* a verified one. This proposal adds two optional frontmatter keys for cross-concept typed relationships, each with its query-time semantics written in, to carry exactly that. They differ on the one axis that drives consumer behaviour at retrieval time: **directionality**.
...
**Load-bearing invariant — exclusion follows affirmation.** An automated writer may propose a supersession; only the appended, attributed resolution event puts it in force. Where a producer runs the community affirmation-status axis (not v0.2 §5.4 `status` — see "A note on the word" below), the retrieval policy reads as exclude on its `superseded` value, which is reached only through that event, never through a flag written onto the incumbent directly. Where no such axis exists, the affirmation still lives in the append-only resolution event, not in a timestamp or a `status` value. A bare `supersedes` pointer MUST NOT remove the incumbent from in-force retrieval: an unaffirmed successor cannot silently displace a live rule, and the appended resolution event stays load-bearing rather than optional.
...
**Where the resolution event lives — `log.md`.** For a consumer to enforce the split above, the resolution event needs a concrete, checkable home in the bundle. That home is the bundle's `log.md` — the reserved, append-only history file the spec already defines (§9 in v0.2; §7 in v0.1). Both resolution dispositions, supersession and dismissal, write their event there: an entry dated by the `valid_until` it corresponds to, attributing the affirmation and referencing both the incumbent's and the successor's stable concept ids per #120, so the event survives a path move on either side. A producer MAY hold this history per-record internally; on export it projects to `log.md`, which is the copy a consumer reads. Where a producer runs the community affirmation-status axis, the same affirmation is also legible from its `superseded` value; otherwise `log.md` is the single instruction behind "check for a resolution event."
...
## Composition with v0.2 core `status`

v0.2 (#227) added a concept-level lifecycle in §5.4: `status: draft | stable | deprecated`, absent meaning `stable`. These edges compose with it, and the relationship is complementary rather than overlapping: **`status` says whether a concept is current; the edge says where current lives.** Neither implies the other. A concept MAY carry `status: deprecated` with no `supersedes` edge (retired with no replacement), or a `supersedes` edge with no status change (a lineage record whose displacement has not been affirmed — see the invariant above). Both are readable independently, and a consumer that understands only one of them still behaves correctly.

**The gap this fills.** §5.4 defines `deprecated` as "kept for links and history; no longer current," but nothing in frontmatter points from the deprecated concept to its replacement. v0.2's own `acme_retail` sample deprecates `gross-margin-legacy.md` and names the current definition only in prose. A consumer filtering out deprecated concepts — the pattern the release recommends — drops the only breadcrumb to the current definition at the moment it acts on the signal. `supersedes` is the machine-readable half of that lifecycle story. The spec itself reaches for the vocabulary: §13.1 describes both of its v0.1→v0.2 renames as "superseded by."

**Trust tiers make contest legibility load-bearing.** §5.3 derives a concept's trust tier from its `verified` events, and verification is doc-level and stored (§10.6). So a concept human-reviewed in June can be contested in July with its tier intact. Without a typed `contested_by` edge, a tier-filtering consumer surfaces that concept as human-reviewed and never sees the dispute. This is precisely the case the exclusion-follows-affirmation invariant exists for: affirmation is an event, and a later contest must remain visible after it.

**A note on the word.** This document uses `status` only in v0.2's §5.4 sense. The community affirmation-status axis discussed on #148 (`proposed | accepted | superseded | deprecated`, per @inkxel; automated writers mechanically capped at `proposed`) is a **separate, producer-side axis** that predates v0.2 and is not proposed here. Where a producer runs it, the edges compose with it the same way: superseding an already-accepted concept is an append — the successor enters as `proposed` carrying the `supersedes` edge, and affirming the successor is what appends the resolution event to the incumbent. The incumbent is never edited directly, so the writer cap survives composition. The dismissal disposition is a resolution event, not a status value. Resolution state, affirmation state, and v0.2 `status` are orthogonal axes; a concept may hold a value on each.

## Relationship to adjacent proposals

- **v0.2 core (#227).** `status` (§5.4) marks whether a concept is current; these edges say what replaced it or what disputes it. `generated` / `verified` and the derived trust tiers (§5.2–5.3) grade a concept's provenance; the edges relate concepts to each other. Neither surface duplicates the other, and both are readable independently — see the composition section above.
...
Both keys are optional and additive. A bundle carrying them remains valid under §11 (parseable frontmatter, non-empty `type`). A consumer that does not recognise the keys preserves them on round-trip per §4.1 and treats the referenced concepts as ordinary linked items. A typed-aware consumer applies the query-time semantics above. Because v0.2 is additive over v0.1 and `type` remains the only always-required field, these keys are conformant against either version.
